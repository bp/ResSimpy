    ...
    def load_output_requests(self):
        ...
        for index, line in enumerate(file_as_list):
            ...
            if ss_start_index > -1 and nfo.check_token('ENDSPREADSHEET', line):
                ss_end_index = index
                list_of_output_requests = self.__get_output_request(file_as_list[ss_start_index:ss_end_index],
                                                                    date=current_date,
                                                                    output_type=OutputType.SPREADSHEET)
                ss_output_requests.extend()

            if nfo.check_token('OUTPUT', line):
                array_start_index = index+1
            if array_start_index > -1 and nfo.check_token('ENDOUTPUT', line):
                array_end_index = index+1
                list_of_output_requests = self.__get_output_request(file_as_list[array_start_index:array_end_index],
                                                                    date=current_date,
