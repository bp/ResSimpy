...
def load_connections(surface_file: NexusFile, start_date: str, default_units: UnitSystem) -> Sequence[
    NexusNodeConnection]:
    """ Loads all connections from a given surface file.
    ...
    Returns:
        Sequence[NexusNodeConnection]: a list of NexusNodeConnections from the surface_file provided
    """

    current_date = start_date
    nexus_nodes_list: list[NexusNodeConnection] = []
    file_as_list = surface_file.get_flat_list_str_file()
    node_map = NexusNodeConnection.get_nexus_mapping()
    node_start = -1
    node_end = -1
    property_dict: dict = {}
    for index, line in enumerate(file_as_list):
        ...
        if nfo.check_token('NODECONS', line):
            node_start = index + 1
        if node_start > 0 and nfo.check_token('ENDNODECONS', line):
            node_end = index
        if 0 < node_start < node_end:
            node_table = nfo.load_table_to_objects(file_as_list=file_as_list, row_object=NexusNodeConnection,
