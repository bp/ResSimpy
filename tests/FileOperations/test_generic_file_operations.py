...
"""
    ...
    expected_base_file = File(location=expected_location, include_locations=expected_includes_list,
    ...
    assert file_with_includes.file_content_as_list == expected_base_file.file_content_as_list
    assert file_with_includes == expected_base_file


def test_generate_file_include_structure_nested_includes_nexus_line_continuation(mocker: MockerFixture, recwarn):
    # Arrange
    file_path = '/path/of/test_file_path.dat'
...
something after extended with >     
a greater than symbol"""

    expected_file_contents = """basic_file 
INCLUDE inc_file1.inc 

something after extended with a greater than symbol"""

    include_file_contents = f"""inc file contents 
INCLUDE '/path/to/inc_file_2.inc'
"""
    ...
    nexus_file_include2 = NexusFile(location='/path/to/inc_file_2.inc', include_locations=[], origin=inc_file_1_full_path,
                               include_objects=None, file_content_as_list=[include_file_contents_2])
    nexus_file_include1 = NexusFile(location='inc_file1.inc', include_locations=['/path/to/inc_file_2.inc'],
                               origin=file_path, include_objects=[nexus_file_include2],
                               file_content_as_list=include_file_contents.splitlines(keepends=True))

    expected_file_content_as_list = expected_file_contents.splitlines(keepends=True)
    ...
    # Act
    file_with_includes = File.generate_file_include_structure(simulator_type=NexusFile, file_path=file_path)
